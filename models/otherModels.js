...
const KesejahteraanModel = {
    ...
    getAllEnriched: async (userId = null, isPrasejahteraOnly = false, options = {}) => {
        ...
        let sql = `
            SELECT 
                p.*, 
                m.nama as nama_penerima, m.nik as nik_penerima, m.jenis_kelamin,
        ...
        if (isPrasejahteraOnly) {
            conditions.push("LOWER(p.kategori_sosial) LIKE '%pra sejahtera%'");
        }
    ...
    create: async (data) => {
        const { member_id, kk_id, income_per_month, house_condition, access_listrik_air, kategori_sosial, kriteria, tingkat_sosial, assessment_notes, keterangan, assessed_by } = data;
        const [res] = await db.query(
            `INSERT INTO kesejahteraan (member_id, kk_id, income_per_month, house_condition, access_listrik_air, kategori_sosial, kriteria, tingkat_sosial, assessment_notes, keterangan, assessed_by) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
            [member_id, kk_id, income_per_month, house_condition, access_listrik_air, kategori_sosial, kriteria, tingkat_sosial, assessment_notes, keterangan, assessed_by]
        );
        return res.insertId;
    },
