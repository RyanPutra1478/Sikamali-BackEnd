...
const PublicService = {
    getLandingStats: async (desa = null) => {
        ...
        const [praRes] = await db.query(`
            ...
            WHERE (LOWER(p.kategori_sosial) LIKE '%pra sejahtera%' OR LOWER(p.kategori_sosial) LIKE '%prasejahtera%')
            ${desa ? 'AND k.desa = ?' : ''}
        `, params);

        const [mandiriRes] = await db.query(`
            SELECT COUNT(*) as count 
            FROM kesejahteraan p
            JOIN kk k ON p.kk_id = k.id
            WHERE (LOWER(p.kategori_sosial) LIKE '%mandiri%' OR LOWER(p.kategori_sosial) LIKE '%sejahtera mandiri%')
            ${desa ? 'AND k.desa = ?' : ''}
        `, params);
        ...
        const [sejRes] = await db.query(`
            ...
            WHERE ((LOWER(p.kategori_sosial) NOT LIKE '%pra sejahtera%' AND LOWER(p.kategori_sosial) NOT LIKE '%prasejahtera%' AND LOWER(p.kategori_sosial) NOT LIKE '%mandiri%') OR p.kategori_sosial IS NULL)
            ${desa ? 'AND k.desa = ?' : ''}
        `, params);
        ...
        return {
            ...
            keluargaSejahteraMandiri: mandiriRes[0].count,
    ...
    getVillageComparison: async () => {
        ...
        for (const v of villages) {
            ...
            const [praRes] = await db.query("SELECT COUNT(*) as count FROM kesejahteraan p JOIN kk k ON p.kk_id = k.id WHERE (LOWER(p.kategori_sosial) LIKE '%pra sejahtera%' OR LOWER(p.kategori_sosial) LIKE '%prasejahtera%') AND k.desa = ?", params);
            const [mandiriRes] = await db.query("SELECT COUNT(*) as count FROM kesejahteraan p JOIN kk k ON p.kk_id = k.id WHERE (LOWER(p.kategori_sosial) LIKE '%mandiri%' OR LOWER(p.kategori_sosial) LIKE '%sejahtera mandiri%') AND k.desa = ?", params);
            const [sejRes] = await db.query("SELECT COUNT(*) as count FROM kk k LEFT JOIN kesejahteraan p ON k.id = p.kk_id WHERE ((LOWER(p.kategori_sosial) NOT LIKE '%pra sejahtera%' AND LOWER(p.kategori_sosial) NOT LIKE '%prasejahtera%' AND LOWER(p.kategori_sosial) NOT LIKE '%mandiri%') OR p.kategori_sosial IS NULL) AND k.desa = ?", params);
            const [empRes] = await db.query(`
                SELECT 
                    COUNT(*) as total_angkatan,
                    SUM(CASE WHEN LOWER(status_kerja) LIKE '%bekerja%' AND LOWER(status_kerja) NOT LIKE '%belum%' THEN 1 ELSE 0 END) as sudah_bekerja,
