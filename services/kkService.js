...
const KKService = {
    createKK: async (userId, kkData, ip) => {
        ...
        // Map 'zona' (from FE) to 'zona_lingkar_tambang' (DB) if present
        if (!restData.zona_lingkar_tambang && restData.zona) {
            restData.zona_lingkar_tambang = restData.zona;
            delete restData.zona;
        }
    ...
    addMember: async (userId, memberData, ip) => {
        ...
        if (memberData.tanggal_lahir === '') memberData.tanggal_lahir = null;
        if (memberData.tanggal_perkawinan === '') memberData.tanggal_perkawinan = null;
    ...
    updateMember: async (userId, userRole, memberId, updateData, ip) => {
        ...
        for (const col of validColumns) {
            if (updateData.hasOwnProperty(col)) {
                let value = updateData[col];
                if ((col === 'tanggal_lahir' || col === 'tanggal_perkawinan') && value === '') {
                    value = null;
                }
                sanitizedData[col] = value;
            }
        }

        await MemberModel.update(memberId, sanitizedData);
    ...
    updateKK: async (userId, userRole, kkId, kkData, ip) => {
        ...
        // Map 'ring' or 'zona' to 'zona_lingkar_tambang' if 'zona_lingkar_tambang' is not present
        if (!updateData.zona_lingkar_tambang) {
            if (kkData.ring) updateData.zona_lingkar_tambang = kkData.ring;
            else if (kkData.zona) updateData.zona_lingkar_tambang = kkData.zona;
        }
